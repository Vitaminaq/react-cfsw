...
class ArticListItem extends Component<ArticListItemProps, any> {
    ...
    toDetail = async():Promise<any> => {
        ...
        const res: API.ChatRoom.View.Response =
            await this.props.saveView({id: this.props.item.articId});
        if (res.data === 'ok') {
                ...
                <Link className="list-dom" onClick={this.toDetail} to='#'>
                    ...
                    <div className="oparatenum">
                        <div className="hasborder">
                            <img src={require('./images/view.svg')} alt="view"/>{item.viewnum}
                        </div>
                        <div className="hasborder">
                            <img src={require('./images/comment.svg')} alt="comment"/> {item.commentnum}
                        </div>
                        <div>
                            <img src={require('./images/click.svg')} alt="click"/>{item.clicknum}
                        </div>
                    </div>
                </Link>
            </li>
