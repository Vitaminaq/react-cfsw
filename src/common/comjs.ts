...
export const timestampToDateTime = (value: number): string | undefined => {
	if (!value) return;
 ...
 * @param str 时间戳
 */
export const timeFromNow = (str: number): undefined | string => {
	if (!str) return;
...
// 时间格式
export const Time = function(str: number | string): undefined | string {
	if (!str) return;
	let date: Date = new Date(Number(str));
